...
namespace CodingSeb.ExpressionEvaluator
{
    ...
    public partial class ExpressionEvaluator
    {
        ...
        protected IDictionary<string, ExpressionOperator> operatorsDictionary = new Dictionary<string, ExpressionOperator>(StringComparer.Ordinal)
        {
           ...
           // { ">", ExpressionOperator.Greater },
            ...
            { "^", ExpressionOperator.TopValue },
            { "<", ExpressionOperator.LowValue },
            { ">", ExpressionOperator.HighValue },
        };
            ...
            new List<IDictionary<ExpressionOperator, Func<dynamic, dynamic, object>>>()
        {
            new Dictionary<ExpressionOperator, Func<dynamic, dynamic, object>>()
            {
                ...
                {ExpressionOperator.TopValue, (dynamic _, dynamic right) => (right & 0xff0000) >> 16},
                {ExpressionOperator.HighValue, (dynamic _, dynamic right) => (right & 0xff00) >> 8},
                {ExpressionOperator.LowValue, (dynamic _, dynamic right) => right & 0xff },
            },
            new Dictionary<ExpressionOperator, Func<dynamic, dynamic, object>>()
            {
        ...
        protected virtual bool EvaluateNumber(string expression, Stack<object> stack, ref int i)
        {
                ...
                || stack.Peek() is ExpressionOperator))
            {
                ...
                var (baseValue, skip) = otherBaseMatch.Groups["type"].Value switch
                {
                    "$" => (16, 1),
                    "x" => (16, 2),
                    "%" => (2, 1),
                    "b" => (2, 2),
                    _ => throw new Exception($"Unhandled base type {otherBaseMatch.Groups["type"].Value}")
                };

                if (otherBaseMatch.Groups["sign"].Success)
                {
                    string value = otherBaseMatch.Groups["value"].Value.Replace("_", "").Substring(skip);
                    stack.Push(otherBaseMatch.Groups["sign"].Value.Equals("-") ? -Convert.ToInt32(value, baseValue) : Convert.ToInt32(value, baseValue));
                }
                else
                {
                    stack.Push(Convert.ToInt32(otherBaseMatch.Value.Replace("_", "").Substring(skip), baseValue));
                }
    ...
    public partial class ExpressionOperator : IEquatable<ExpressionOperator>
    {
        ...
        public static readonly ExpressionOperator TopValue = new ExpressionOperator();
        public static readonly ExpressionOperator HighValue = new ExpressionOperator();
        public static readonly ExpressionOperator LowValue = new ExpressionOperator();
